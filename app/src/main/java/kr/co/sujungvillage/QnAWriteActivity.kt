...
class QnAWriteActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnUpload.setOnClickListener {
            ...
            // 질문 작성 API 연결
            RetrofitBuilder.qnaApi.myqWrite(token, questionInfo)
                .enqueue(object : Callback<MyqWriteResultDTO> {
                    override fun onResponse(
                        call: Call<MyqWriteResultDTO>,
                        response: Response<MyqWriteResultDTO>
                    ) {
                        Log.d("MY_QUESTION_WRITE", "질문 작성 성공")
                        Log.d("MY_QUESTION_WRITE", response.body().toString())
                        Toast.makeText(this@QnAWriteActivity, "질문이 등록되었습니다.", Toast.LENGTH_SHORT)
                            .show()
                        setResult(RESULT_OK)
                        finish()
                    }
